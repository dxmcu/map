...
double CommonArea::origin_x = 0;

double CommonArea::origin_y = 0;

int CommonArea::x_cells = 0;

int CommonArea::y_cells = 0;

cv::Mat CommonArea::map = cv::Mat(1, 1, CV_8U);

std::vector< std::vector<bool> > CommonArea::vis_(0);


void CommonArea::OG2cvMat(nav_msgs::OccupancyGrid &og, cv::Mat &mat) {
...
void CommonArea::addOG2Mat(nav_msgs::OccupancyGrid &og) {
#define CA_UNKNOW -1
#define CA_NO 100
#define CA_YES 0
...
cv::Rect CommonArea::findMinRect(const cv::Mat1b& src)
{
...
cv::RotatedRect CommonArea::largestRectInNonConvexPoly(const cv::Mat1b& src)
{
...
//根据map的数据，得到公共区域
std::vector<cv::Mat> CommonArea::getContor()
{
...
std::vector< std::vector< geometry_msgs::Point > > CommonArea::commonOccupancyGrid(nav_msgs::OccupancyGrid &og1, nav_msgs::OccupancyGrid &og2)
{
...
void CommonArea::commonPoints()  //直接求2点云?
{
    //TODO
}
