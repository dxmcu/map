...
PointMatcher<float>::TransformationParameters MapDbNode::transformationInverse(const PointMatcher<float>::TransformationParameters inTransformation)
{
...
//double
void MapDbNode::transform(const Eigen::Matrix4d & trans)
{
    Eigen::Matrix4f T;
    for (int i = 0; i < 3; ++ i)
        for (int j = 0; j < 3; ++ j)
...
//double
void MapDbNode::invTransform(const Eigen::Matrix4d & trans)
{
    Eigen::Matrix4f T;
    for (int i = 0; i < 3; ++ i)
        for (int j = 0; j < 3; ++ j)
...
//float
void MapDbNode::transform(const Eigen::Matrix4f & trans)
{
...
//float
void MapDbNode::invTransform(const Eigen::Matrix4f & trans)
{
    PointMatcher<float>::ICP icp;
    icp.setDefault();
...
void MapDbNode::loadGlobalMapFromPCD(const std::string & pcd_name)  //从pcd文件获取global map
{
...
void MapDbNode::pos2cloud(const geometry_msgs::PoseWithCovarianceStamped &pose, sensor_msgs::PointCloud2 &cloud)
{
...
bool MapDbNode::retrieve(const geometry_msgs::PoseStamped &mapLoc,
